"""evosax LM-MA-ES runner for SiGe JAX objectives."""
...
import numpy as np
from evosax.algorithms.distribution_based.lm_ma_es import LM_MA_ES
...
def run_sige_evosax_lm_ma_es(
    ...
    """Run evosax LM-MA-ES on a prepared SiGe model in unit-cube space.

    ``generations`` follows the SciPy/DE-style maxiter convention used by the
    demo script: sampled LM-MA-ES populations are evaluated for generation
    indices 0 through ``generations``.
    """
    if population_size < 2:
        raise ValueError("LM-MA-ES requires population_size >= 2")
    if generations < 0:
        raise ValueError("generations must be >= 0")
    if std_init <= 0:
        raise ValueError("std_init must be > 0")
    if std_min < 0:
        raise ValueError("std_min must be >= 0")
    if std_max <= 0:
        raise ValueError("std_max must be > 0")
    if std_min > std_max:
        raise ValueError("std_min must be <= std_max")
    ...
    if verbose:
        ...
        print(f"Initial LM-MA-ES std: {float(std_init):.6g}")
    ...
    if polish:
        if verbose:
            print("Polishing best JAX LM-MA-ES solution with bounded L-BFGS-B")
