"""evosax differential-evolution runner for SiGe JAX objectives."""
...
import numpy as np
from evosax.algorithms.population_based.differential_evolution import DifferentialEvolution
...
def run_sige_evosax_de(
    ...
    """Run evosax DE on a prepared SiGe model in normalized unit-cube space."""
    ...
    population = jax.random.uniform(
        pop_key, (population_size, n_params), dtype=jnp.float32
    )
    population = population.at[0].set(jnp.asarray(default_unit, dtype=jnp.float32))

    t0 = time.perf_counter()
    fitness = objective_batch(population).block_until_ready()
    ...
    params = algorithm.default_params.replace(
    ...
    initial_seeded_fun = float(fitness[0])
    initial_best_fun = float(jnp.min(fitness))
    if verbose:
        print(f"Parameters: {n_params}")
        print(f"Initial seeded GF: {initial_seeded_fun:.6f}")
        print(f"Initial population best GF: {initial_best_fun:.6f}")
    ...
    for generation in range(1, generations + 1):
        ...
        best_fitness = float(jnp.min(state.fitness))
        (
            converged,
            population_std,
            population_mean,
            convergence_threshold,
        ) = _population_converged(state.fitness, tol, atol)
        ...
        if verbose and (generation % print_every == 0 or generation == generations):
            print(f"Generation {generation:04d}: best GF {best_fitness:.6f}")
        if converged:
            stop_reason = (
    ...
    best_unit = np.asarray(jnp.clip(algorithm.get_best_solution(state), 0.0, 1.0))
    best_x = unit_to_physical_np(best_unit, problem.lower, problem.upper)
    best_fun = float(jnp.min(state.fitness))
    nfev = population_size * (generations_completed + 1)
    ...
    if polish:
        if verbose:
            print("Polishing best JAX DE solution with bounded L-BFGS-B")
