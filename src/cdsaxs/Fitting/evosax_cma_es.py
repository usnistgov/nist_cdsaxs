"""evosax CMA-ES runner for SiGe JAX objectives."""
...
def run_sige_evosax_cma_es(
    ...
    """Run evosax CMA-ES on a prepared SiGe model in normalized unit-cube space.

    ``generations`` follows the SciPy/DE-style maxiter convention used by the
    demo script: sampled CMA-ES populations are evaluated for generation
    indices 0 through ``generations``.
    """
    if population_size < 2:
        raise ValueError("CMA-ES requires population_size >= 2")
    if generations < 0:
    ...
    if verbose:
        ...
        print(f"Initial CMA-ES std: {float(std_init):.6g}")
    ...
    if polish:
        if verbose:
            print("Polishing best JAX CMA-ES solution with bounded L-BFGS-B")
