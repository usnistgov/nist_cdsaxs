"""evosax CR-FM-NES runner for SiGe JAX objectives."""
...
import numpy as np
from evosax.algorithms.distribution_based.cr_fm_nes import CR_FM_NES
...
def run_sige_evosax_cr_fm_nes(
    ...
    """Run evosax CR-FM-NES on a prepared SiGe model in unit-cube space.

    ``generations`` follows the SciPy/DE-style maxiter convention used by the
    demo script: sampled CR-FM-NES populations are evaluated for generation
    indices 0 through ``generations``.
    """
    if population_size < 2:
        raise ValueError("CR-FM-NES requires population_size >= 2")
    if population_size % 2:
        raise ValueError("CR-FM-NES requires an even population_size")
    if generations < 0:
    ...
    params = algorithm.default_params.replace(std_init=float(std_init))
    ...
    if verbose:
        ...
        print(f"Initial CR-FM-NES std: {float(std_init):.6g}")
    ...
    for generation in range(0, generations + 1):
        ...
        if verbose and (generation % print_every == 0 or generation == generations):
            print(
                ...
                f"best GF {best_fun:.6f}, std {float(state.std):.6g}"
            )
        if converged:
            stop_reason = (
    ...
    if polish:
        if verbose:
            print("Polishing best JAX CR-FM-NES solution with bounded L-BFGS-B")
