...
class CDSAXS_Model:
        ...
        try:
            ...
            if hasattr(self, 'geometry') and self.geometry == 'trapezoid':
                try:
                    ...
                    self.SimInt_Initial = self.SimInt.copy()
    ...
    def _print_parameter_changes(self, initial_model_params):
        ...
        # Get the maximum number of trapezoids from either model
    ...
    def plot_trapezoid_structure(self):
        """
    ...
    def plot_trapezoid_structure(self):
        """
        Plots the trapezoid structure from model parameters
        """
        ...
        # Plot base
        plt.plot([0, trapezoids[0]['width']], [0, 0], 'k-', linewidth=2)
        ...
        for i in range(layers + 1):
            ...
            plt.plot([x_left, x_right], [height, height], 'k-', linewidth=2)
            
            if i < layers:
                x_next_left = (trapezoids[0]['width'] - trapezoids[i+1]['width']) / 2
                x_next_right = x_next_left + trapezoids[i+1]['width']
                plt.plot([x_left, x_next_left], [height, height + trapezoids[i]['height']], 'k-', linewidth=2)
                plt.plot([x_right, x_next_right], [height, height + trapezoids[i]['height']], 'k-', linewidth=2)
    ...
    def PlotQzCutArray(self, cut_index=None, SimInt=None, log_scale='yes'):
        """
    ...
    def PlotQzCut(self, numbercuts=None, SimInt=None, scale='yes', scale_factor=50.0):
        """
        Plots intensity vs Qz for multiple Qx cuts on the same figure
        with sequential scaling to improve clarity.
        
        Parameters:
        -----------
        numbercuts : int or None, optional
            Number of cuts to plot
            If None, plots all available cuts
        SimInt : numpy.ndarray, optional
            Simulated intensity to plot alongside measured data
            If None, uses self.SimInt if available
        scale : str, optional
            Whether to apply sequential scaling ('yes' or 'no')
        scale_factor : float, optional
            Base factor for sequential scaling (default: 50.0)
            Each cut's intensity is divided by scale_factor^(i+1)
        ...
        # Determine simulation data to use
        if SimInt is None and hasattr(self, 'SimInt'):
            SimInt = self.SimInt
        
        # Determine number of cuts to plot
        if numbercuts is None:
            numbercuts = self.Intensity.shape[1]
        else:
            numbercuts = min(numbercuts, self.Intensity.shape[1])
        
        # Create a deep copy of the data to avoid modifying the original
        import copy
        Intensity_plot = copy.deepcopy(self.Intensity[:, :numbercuts])
        SimInt_plot = copy.deepcopy(SimInt[:, :numbercuts]) if SimInt is not None else None
        
        # Apply sequential scaling if requested
        if scale.lower() == 'yes':
            for i in range(numbercuts):
                scale_value = scale_factor**(i+1)
                Intensity_plot[:, i] = Intensity_plot[:, i] / scale_value
                if SimInt_plot is not None:
                    SimInt_plot[:, i] = SimInt_plot[:, i] / scale_value
        
        # Create figure
        plt.figure(figsize=(10, 6))
        
        # Create empty plots for legend
        exp_line, = plt.semilogy([], [], 'o', color='blue', label='Experimental')
        if SimInt_plot is not None:
            sim_line, = plt.semilogy([], [], '-', color='black', label='Simulated')
        
        # Plot each cut
        for i in range(numbercuts):
            # Determine color for experimental data
            ...
            # Plot simulated intensity if available - using black for all
            if SimInt_plot is not None:
                plt.semilogy(
                    self.Qz[:, i], 
                    SimInt_plot[:, i], 
        ...
        plt.ylabel('Intensity (a.u.)')
        if scale.lower() == 'yes':
            plt.title(f'Intensity vs Qz (scaled by factor of {scale_factor})')
        else:
            plt.title('Intensity vs Qz')
        
        # Add the simplified legend
        legend_elements = [exp_line]
        if SimInt_plot is not None:
            legend_elements.append(sim_line)
        ...
        return plt.gca()
    def plot_intensity_map(self, intensity=None, log_scale=True, title=None):
        """
        Creates a 2D colormap of intensity
        
        Parameters:
        -----------
        intensity : numpy.ndarray, optional
            Intensity data to plot
            If None, uses self.Intensity
        log_scale : bool, optional
            Whether to use logarithmic scale for intensity
        title : str, optional
            Title for the plot
            If None, uses a default title
        """
        if intensity is None:
            if not hasattr(self, 'Intensity'):
                raise AttributeError("Missing required attribute: Intensity")
        ...
        plot_data = np.log10(intensity) if log_scale else intensity
        ...
        # Set title
        if title is None:
            title = 'log(Intensity)' if log_scale else 'Intensity'
        plt.title(title)
    ...
    def compare_measured_simulated(self):
        """
        Creates a side-by-side comparison of measured and simulated intensities
        """
        if not hasattr(self, 'SimInt') or self.SimInt is None:
            raise AttributeError("Missing required attribute: SimInt")
        
        plt.figure(figsize=(16, 6))
        
        # Plot measured intensity
        plt.subplot(1, 3, 1)
        self.plot_intensity_map(self.Intensity, title='Measured log(Intensity)')
        
        # Plot simulated intensity
        plt.subplot(1, 3, 2)
        self.plot_intensity_map(self.SimInt, title='Simulated log(Intensity)')
        
        # Plot residuals
        plt.subplot(1, 3, 3)
        residuals = np.log10(self.Intensity) - np.log10(self.SimInt)
        ...
        plt.title(f'Residuals (GF={self.GF:.2f})')
        plt.xlabel('Qx (nm^-1)')
        plt.ylabel('Qz (nm^-1)')
        plt.colorbar(im, label='log(Measured) - log(Simulated)')
